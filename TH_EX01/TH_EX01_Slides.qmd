...
p_age_lr <- ggplot(by_age, aes(x = age_band, y = loss_ratio)) +
  geom_col(aes(fill = loss_ratio > 0.7), show.legend = FALSE, width = 0.7) +
  geom_text(aes(label = percent(loss_ratio, accuracy = 0.1)),
            vjust = -0.4, size = 2.8) +
  ...
  labs(title = "Loss ratio by driver age band", x = NULL, y = "Loss ratio") +
  theme_minimal(base_size = 9) +
  theme(plot.title  = element_text(face = "bold", colour = "#2C5282"),
        axis.text.x = element_text(angle = 30, hjust = 1, size = 7))

p_age_prem <- ggplot(by_age, aes(x = age_band, y = avg_premium)) +
  geom_col(fill = "#2C5282", width = 0.7) +
  geom_text(aes(label = dollar(avg_premium, prefix = "\u20ac", accuracy = 1)),
            vjust = -0.4, size = 2.8) +
  scale_y_continuous(labels = label_dollar(prefix = "\u20ac"),
                     expand = expansion(mult = c(0, 0.18))) +
  labs(title = "Average premium per exposure", x = NULL, y = "EUR") +
  theme_minimal(base_size = 9) +
  theme(plot.title  = element_text(face = "bold", colour = "#2C5282"),
        axis.text.x = element_text(angle = 30, hjust = 1, size = 7))
...
[Full report and code at ISSS608 Take home Exercise 1]{.thank-you-sub}
[← Technical Report](TH_EX01.html){.title-back-link}
:::
