...
## [3 Programming Interactive Data Visualisation with R]{style="color:  #4682B4; font-size: 38px;"}

## [3.1 Learning Outcome]{style="color:  #2C5282; font-size: 28px;"}
...
## [3.2 Getting Started]{style="color:  #2C5282; font-size: 28px;"}
...
- [ggiraph](https://davidgohel.github.io/ggiraph/) turns ggplot2 graphics interactive.

- [plotly](https://plotly.com/r/) uses Javascript library for having interactivity such as zoom, pan, hover and selection out of box.

- [DT](https://rstudio.github.io/DT/) creates interactive HTML tables for sorting, search, or paginate.

- [patchwork](https://patchwork.data-imaginist.com/) combines multiple ggplots side by side that helps create **coordinated multiple views**.

- [tidyverse](https://tidyverse.org/) to use `readr` for import, `dplyr` for wrangling, and `ggplot2` as the static base.

## [3.3 Importing Data]{style="color:  #2C5282; font-size: 28px;"}

The file *Exam_data* is used for this exercise. The file can be imported using the [*read_csv()*](https://readr.tidyverse.org/reference/read_delim.html) function of readr package.

```{r}
exam_data <- read_csv("data/Exam_data.csv")
```

## [3.4 Interactive Data Visualisation - ggiraph Methods]{style="color:  #2C5282; font-size: 28px;"}

Interactivity is made with [ggiraph geometries](https://davidgohel.github.io/ggiraph/reference/index.html#section-interactive-geometries) that understands three aesthetics in line with Shneiderman's mantra.

| Aesthetics | User action                       | Shneiderman mantra |
|------------|-----------------------------------|--------------------|
| `tooltip`  | hover                             | details-on-demand  |
| `data_id`  | hover (highlight linked elements) | filter / focus     |
| `onclick`  | click                             | drill-down         |

It can be used to add interactive elements to your [Shiny](https://shiny.posit.co/r/getstarted/shiny-basics/lesson1/) applications.

## [3.4.1 Tooltip Effect with tooltip aesthetic]{style="color:  #366092; font-size: 24px;"}

Below shows a typical code chunk to plot an interactive statistical graph by using **ggiraph** package. First, an ggplot object will be created. Next, [`girafe()`](https://davidgohel.github.io/ggiraph/reference/girafe.html) of **ggiraph** will be used to create an interactive svg object.
...
|------------------------|------------------------|------------------------|
| `geom_point_interactive()` | scatterplot points | bivariate continuous |
| `geom_col_interactive()` / `geom_bar_interactive()` | bars | categorical comparisons |
| `geom_line_interactive()` | lines | time series, trends |
| `geom_boxplot_interactive()` | boxplots | distribution by group |
| `geom_dotplot_interactive()` | stacked dots | small-n distributions |
| `geom_jitter_interactive()` | points with random noise added | overlapping categorical-vs-continuous data; alternative to boxplot when n is small enough to show every observation |
| `geom_tile_interactive()` | heatmap tiles | matrices, calendars |
| `geom_polygon_interactive()` / `geom_sf_interactive()` | filled shapes / map features | geospatial |
...
`stackgroup = TRUE` in a dotplot means that the dots are stacked vertically when their x-values fall in the same bin (so multiple students with MATHS = 75 will be stacked on top of each other).
...
`width_svg = 6, height_svg = 6*0.618` are considered to be the golden ratio. [Alberto Cairo](https://en.wikipedia.org/wiki/Alberto_Cairo) and [Claus Wilke](https://clauswilke.com/) both use this default for aesthetic balance.
:::

## [3.5 Interactivity]{style="color:  #2C5282; font-size: 28px;"}
...
## Dotdensity + y-axis Name and Break + Tooltip Class + binwidth = 3
...
p <- ggplot(data=exam_data, 
  ...
  geom_dotplot_interactive(
    ...
    binwidth = 3, 
    method = "dotdensity") +
  scale_y_continuous(name= "Score", breaks = c(0, 0.5, 1))
girafe(
  ggobj = p,
  width_svg = 6,
  height_svg = 6*0.618
...
- The y-axis has the label "Score" as well as the ticks marks and tick labels. Changing `break = c(0, 0.5, 1)` to `break = waiver()` lets ggplot2 decide the scale.

- On hover, the ID (e.g Student099) is changed to the CLASS (e.g. 3F).
...
## [3.6 Displaying Multiple Information on Tooltip]{style="color:  #2C5282; font-size: 28px;"}
...
## [3.6.1 Customising Tooltip Style]{style="color:  #366092; font-size: 24px;"}
...
## [3.6.2 Displaying Statistics on Tooltip]{style="color:  #366092; font-size: 24px;"}
...
p1 <- gg_point <- ggplot(data=exam_data, 
                   aes(x = RACE),
) +
  stat_summary(aes(y = MATHS, 
                   tooltip = after_stat(  
...
If we take a look at the code and decompose it:

- `stat_summary()` computes a summary (`fun.data = mean_se` returns y, ymin, ymax, and mean ± standard error) and plots that.

- `geom = GeomInteractiveCol` uses ggiraph's interactive column geom instead of the default bar. This is what makes the bar respond to hover.

- `after_stat(tooltip(y, ymax))` compute the expression *after* the stat has run, using the stat's output columns. So `y` is the mean, `ymax` is mean + SE, and the custom function formats them.

- The second `stat_summary()` overlays an error bar on top.

## [3.6.3 Hover Effect with data_id aesthetic]{style="color:  #366092; font-size: 24px;"}
...
## [3.6.4 Styling Hover Effect]{style="color:  #366092; font-size: 24px;"}
...
## [3.6.5 Combining Tooltip and Hover Effect]{style="color:  #366092; font-size: 24px;"}
...
## [3.6.6 Click Effect with onclick]{style="color:  #366092; font-size: 24px;"}
...
## [3.6.7 Coordinated Multiple Views with ggiraph]{style="color:  #366092; font-size: 24px;"}
...
## [3.7 Interactive Data Visualisaion - plotly methods!]{style="color:  #2C5282; font-size: 28px;"}

Plotly’s R graphing library creates interactive web graphics from **ggplot2** graphs and/or a custom interface to the (MIT-licensed) JavaScript library [**plotly.js**](https://plotly.com/javascript/) inspired by the grammar of graphics. Different from other plotly platform, plotly is free and open source. It gives you zoom, pan, lasso-select, box-select, hover, and a download button.
...
## [3.7.1 Creating an Interactive Scatter Plot: plot_ly() method]{style="color:  #366092; font-size: 24px;"}
...
## [3.7.2 Working with Visual Variable: plot_ly() method]{style="color:  #366092; font-size: 24px;"}
...
## [3.7.3 Creating an Interactive Scatter Plot: ggplotly() method]{style="color:  #366092; font-size: 24px;"}
...
## [3.7.4 Coordinated Multiple Views with plotly]{style="color:  #366092; font-size: 24px;"}
...
## [3.8 Interactive Data Visualisaion - crosstalk methods!]{style="color:  #2C5282; font-size: 28px;"}
...
## [3.8.1 Interactive Data Table: DT package]{style="color:  #366092; font-size: 24px;"}
...
## [3.8.2 Linked Brushing: crosstalk method]{style="color:  #366092; font-size: 24px;"}
...
## [3.9 Consolidation]{style="color:  #2C5282; font-size: 28px;"}

::: callout-note
## When to use ggiraph vs plotly
...
## [3.9.1 Consolidated ggiraph Plot Example]{style="color:  #366092; font-size: 24px;"}

```{r}
#| echo: false
exam_data <- exam_data |>
  mutate(
    pct_rank = round(percent_rank(MATHS) * 100, 0),
    deviation = round(MATHS - mean(MATHS, na.rm = TRUE), 1),
    rich_tooltip = paste0(
      "<b>", ID, "</b><br>",
      "Class: ", CLASS, "<br>",
      "MATHS: ", MATHS, " (", pct_rank, "th percentile)<br>",
      "vs. cohort mean: ", ifelse(deviation >= 0, "+", ""), deviation
    ),
    onclick = sprintf(
      "window.open('https://www.google.com/search?q=%s')",
      URLencode(paste("Singapore Primary 3", CLASS), reserved = TRUE)
    )
  )
...
p_ggiraph <- ggplot(exam_data, aes(x = MATHS)) +
  geom_dotplot_interactive(
    aes(tooltip = rich_tooltip,
        data_id = CLASS,
        onclick = onclick),
    stackgroups = TRUE,
    binwidth = 1,
  ...
  geom_vline(
    xintercept = mean(exam_data$MATHS, na.rm = TRUE),
    colour = "darkred",
    linetype = "dashed",
    linewidth = 0.5
  ) +
  annotate(
    "text",
    x = mean(exam_data$MATHS) + 1, y = 0.95,
    label = paste0("Cohort mean = ", round(mean(exam_data$MATHS), 1)),
    colour = "darkred", hjust = 0, size = 3.5
  ) +
  coord_cartesian(xlim = c(0, 100)) +
  scale_y_continuous(NULL, breaks = NULL) +
  labs(
    title = "Primary 3 MATHS distribution",
    subtitle = "Hover for student details · same-class dots highlight together · click to search class",
    x = "MATHS score"
  ) +
  theme_minimal()
...
girafe(
  ...
  options = list(
    ...
    opts_hover(css = "fill: #1d9e75; stroke: black; stroke-width: 0.6;"),
...
## [3.9.2 Consolidated plotly Example]{style="color:  #366092; font-size: 24px;"}

```{r}
#| echo: false
# plotly showcase: scatter of MATHS vs ENGLISH coloured by RACE
# Demonstrates: free zoom/pan/lasso, legend toggle, custom HTML tooltip,
# colourblind-safe palette, plotly's native interactivity toolbar

# Pre-compute the hover text
exam_data <- exam_data |>
  mutate(hover_text = paste0(
    "<b>", ID, "</b><br>",
    "Class: ", CLASS, "<br>",
    "Race: ", RACE, " | Gender: ", GENDER, "<br>",
...
p_plotly <- plot_ly(
  ...
  y = ~ENGLISH,
  color = ~RACE,
  colors = "Set2",                       # colourblind-safe override
  text = ~hover_text,
  hoverinfo = "text",
  type = "scatter",
  mode = "markers",
  marker = list(size = 8, opacity = 0.7, line = list(width = 0.5, color = "white"))
) |>
  layout(
    title = list(
      text = "MATHS vs ENGLISH by race<br><sub>Drag to zoom · lasso to select · click legend to toggle</sub>",
      x = 0
    ),
    xaxis = list(title = "MATHS", range = c(0, 100), gridcolor = "#eee"),
    yaxis = list(title = "ENGLISH", range = c(0, 100), gridcolor = "#eee"),
    legend = list(title = list(text = "<b>Race</b>")),
    plot_bgcolor = "white"
  ) |>
  config(
    displaylogo = FALSE,
    modeBarButtonsToRemove = c("autoScale2d", "toggleSpikelines")
  )
...
## [4 Programming Animated Statistical Graphics with R]{style="color:  #4682B4; font-size: 38px;"}

## [4.1 Overview]{style="color:  #2C5282; font-size: 28px;"}
...
## [4.1.1 Basic Concept of Animation]{style="color:  #366092; font-size: 24px;"}
...
## [4.1.2 Terminology]{style="color:  #366092; font-size: 24px;"}
...
## [4.2 Getting Started]{style="color:  #2C5282; font-size: 28px;"}

## [4.2.1 Loading the R packages]{style="color:  #366092; font-size: 24px;"}
...
## [4.2.1 Importing the Data]{style="color:  #366092; font-size: 24px;"}
...
## [4.3 Animated Data Visualisation: gganimate methods]{style="color:  #2C5282; font-size: 28px;"}
...
|------------------------|------------------------|------------------------|
| `transition_*()` | How the data should be spread out and how it relates to itself across time. | `transition_time()`, `transition_states()`, `transition_reveal()`, `transition_manual()` |
| `view_*()` | How the positional scales should change along the animation | `view_follow()`, `view_static()`, `view_step()` |
| `shadow_*()` | How data from other points in time should be presented in the given point in time | `shadow_mark()`, `shadow_wake()`, `shadow_trail()` |
| `enter_*()/exit_*()` | How new data should appear and how old data should disappear during the course of the animation. | `enter_fade()`, `exit_shrink()` |
| `ease_aes()` | How smoothly aesthetics transition between frames. | `'linear'`, `'cubic-in-out'`, `'bounce-out'` |

## [4.3.1 Building a Static Population Bubble Plot]{style="color:  #366092; font-size: 24px;"}
...
## [4.3.2 Building the Animated Bubble Plot]{style="color:  #366092; font-size: 24px;"}
...
The animated bubble chart with examples of `transition_time()`, `transition_state()`.
...
## [4.4 Animated Data Visualisation: plotly]{style="color:  #2C5282; font-size: 28px;"}

In **Plotly R** package, both `ggplotly()` and `plot_ly()` support key frame animations through the `frame` argument/aesthetic. They also support an `ids` argument/aesthetic to ensure smooth transitions between objects with the same id (which helps facilitate object constancy).

## [4.4.1 Building the Animated Bubble Plot: ggplotly() method]{style="color:  #366092; font-size: 24px;"}
...
## [4.4.2 Building the Animated Bubble Plot: plot_ly() method]{style="color:  #366092; font-size: 24px;"}
